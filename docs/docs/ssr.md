...
Execute ʻumi dev` to visit the page, which is rendered by the server.
...
app.use(async (req, res) => {
  ...
  const context = {};
  const { html, error, rootContainer } = await render({
    // You can bring query if necessary
    path: req.url,
    context,
    ...
    // Expand the parameters of getInitialProps in server-side rendering
    // getInitialPropsCtx: {},
...
```ts
{
  ...
  htmlTemplate?: string;
  // Optional, the page content mount node, used in conjunction with htmlTemplate, the default is root
...
```ts
{
  ...
  // Error object, after rendering the error on the server side, the value is not null
  error?: Error;
}
```
...
Two examples are currently based on [koa](https://github.com/umijs/umi/tree/master/examples/ssr-koa) and [egg](https://github.com/umijs/ umi/tree/master/examples/ssr-with-eggjs), the example has built-in dva data flow and internationalization solutions, and the code part has comments, which can be modified by reference.
...
Pre-rendering will not render all pages in dynamic routing by default. If you need to render pages in dynamic routing, configure ʻextraRoutePaths`, for example:
...
In the page, the title can be rendered directly：
...
2. Make environmental judgments through the ʻisBrowser` method provided by umi, for example:
...
1. Because umi internal modules use a lot of ʻalias`, if you do external, you will do path mapping for a large number of modules. When actually used on the server side, some packages will have incorrect paths and can’t be loaded.
...
Check the webpage source code, if the element in the `<div id="root">` DOM is not empty, it is SSR, otherwise it is CSR.
