...
使用 antd 5.17.0 及以上版本时，可配置 `styleProvider: { layer: true }`，将组件样式放入 `@layer antd`，降低样式优先级。请同时配置 `antd.configProvider: {}`（或保留已有配置），以启用内层 `ConfigProvider` 处理图标样式。设置 `layer: false` 可显式关闭，未设置时保留 `StyleProvider` 的默认或继承行为。
