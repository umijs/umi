...
若你需要在不同环境中加载不同配置，这在 Umi 中是根据 [`UMI_ENV`](./guides-env-variables#umi_env) 来实现的，一个不同环境启动的例子：
...
存放 mock 文件，此目录下所有 `.ts` / `.js` 文件会被 mock 服务加载，从而提供模拟数据，使用方法详见 [Mock](./guides-mock) 。
...
当你需要关闭 layout 时可以使用 `layout: false` ，当你需要更多层 layout 时，可以考虑使用 [`wrappers`](./guides-routes#wrappers) ，仅在配置式路由可用：
