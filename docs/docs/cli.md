...
Compile and build web products. It is usually necessary to make specific configuration and environment variable modifications for the deployment environment. For details, please refer to [Deployment](./deployment).
...
The default product output is to the project's `dist` folder, you can modify the configuration `outputPath` to specify the product output directory.
...
> Note: If there is a product file with the same name in `public`, such as `index.html`, the product file will be overwritten.
...
You can specify the development port number by setting the environment variable `PORT`. For more environment variable configuration, please refer to [Environmental Variables](/docs/env-variables).
...
> Note: If you are in a complex network environment such as a VPN or virtual machine, this address is likely to be wrong. You can access the development page by accessing the corresponding port number of your real available `ip`.

## umi generate

The built-in generator function, the built-in type is `page`, which is used to generate the simplest page. Support alias calling `umi g`.

```bash
$ umi generate <type> <name> [options]
```

This command supports extensions. Register through `api.registerGenerator`, you can implement your own commonly used generators through plugins.
...
Quickly view all the `umi` plugins used in the current project.
...
Simple help document for umi command line.
...
By default, the development configuration will be printed. To view the production configuration, you need to specify environment variables:
