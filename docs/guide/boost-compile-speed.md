...
When executing `umi dev` or `umi build`, increase the environment variable `ANALYZE=1` to view the product dependency ratio.
...
Choosing a suitable browser version can reduce a lot of size. For example, if it is configured as follows, it is expected to reduce the size of 90K (not gzip after compression).
