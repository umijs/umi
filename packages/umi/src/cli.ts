...
if (v && parseInt(v.slice(1)) < 10) {
  chalk.red(`Your node ${v} is not supported by umi, please upgrade to 10 or above.`);
  process.exit(1);
} else if (v && parseInt(v.slice(1)) > 14) {
  chalk.yellow(`Your node ${v} is not supported by umi, please use node 10, node 12 or node 14.`);
}
