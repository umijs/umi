...
import { LazyFolderCache } from './LazyFolderCache';
...
export class LazySourceCodeCache {
  ...
  constructor(opts: { cwd: string; cachePath: string }) {
    ...
    this.folderCache = new LazyFolderCache({
      cwd: this.srcPath,
      exts: ['ts', 'js', 'jsx', 'tsx'],
      ...
      onCacheUpdate: async (_cache, events) => {
  ...
  async handleFileChangeEvents(e: FileChangeEvent[]) {
    return this.folderCache.handleFileChangeEvents(e);
  }
