...
import { fsExtra, importLazy, lodash, Mustache, prettier } from '@umijs/utils';
...
export default (api: IApi) => {
  ...
  api.registerMethod({
    ...
    fn(opts: {
      ...
      const isTsFile = isTypeScriptFile(opts.path);
      ...
      if (opts.format !== false) {
        const prettierConfig = prettier.resolveConfig.sync(api.paths.cwd);
        if (prettierConfig) {
...
function formatCode(code: string, ops: { cwd: string; path: string }) {
  const prettierConfig = prettier.resolveConfig.sync(ops.cwd);
  if (prettierConfig) {
