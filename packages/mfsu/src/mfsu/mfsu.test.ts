...
test('mfsu.define be stringified', () => {
  const mfsu = new MFSU({
    implementor: webpack as any,
    buildDepWithESBuild: true,
    depBuildConfig: null,
  ...
  expect(mfsu.opts.define).toStrictEqual({
    foo: JSON.stringify('1'),
    bar: JSON.stringify(1),
  });
});

test('send args by postMessage', () => {
  const mockedFn = jest.fn();
  const startBuildWorker = () => ({
    postMessage: ({ args }) => mockedFn(args),
  });
  const args = {};
  ...
  new MFSU({
    implementor: webpack as any,
    depBuildConfig: null,
    startBuildWorker: startBuildWorker as any,
    strategy: 'eager',
    srcCodeCache: {
      register: () => {},
    },
  ...
  expect(mockedFn).toBeCalledWith(args);
});
