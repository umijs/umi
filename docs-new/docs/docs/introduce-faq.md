...
参考 [目录结构 > loading.tsx](./guides-directory-structure#src-目录)
...
![image](./img/less-error.png)
解法：新版 less 中 `/` 默认被识别为属性简写，通过配置 `lessLoader: { math: 'always' }` 恢复旧版行为（默认将 `/` 用作计算符号）。

## routes 里的 layout 配置选项不生效

layout 配置被移动到了 `app.ts` ，详见 [runtime-config > layout](./api-runtime-config#layout)
...
如果你有将所有 js 产物打包成单 `umi.js` 文件的需求，请关闭 [dynamicImport](./introduce-faq#可以关闭-dynamicimport-吗) 。
...
加载优先级详见 [UMI_ENV](./guides-env-variables#umi_env) ，无论是 `config/config.ts` 还是 `.umirc.ts` 同理。
